...
// Adjust the timeout for each split combination here (in milliseconds)
const int TIMEOUT_PER_TASK_MS = 10000; // Set a 10-second timeout for each split combination
...
void tryMergeAndRecur(const vector<double>& vals,
    ...
    if (chrono::duration_cast<chrono::milliseconds>(now - search_start_time).count() > TIMEOUT_PER_TASK_MS) {
        return; // Timeout, abandon the current search branch
    }

    // Pruning: If the current expression length already exceeds the best solution, no need to continue
    if (hasBest) {
        size_t current_len = 0;
        for(const auto& s : exprs) current_len += s.size();
        if (current_len >= bestSolution.size()) {
            return;
        }
    }
...
int main() {
    ...
    for (int split = 1; split < (int)s.size(); ++split) {
        ...
        for (int mask = 0; mask < ways; ++mask) {
            ...
            // Reset the timer for this combination
            search_start_time = chrono::steady_clock::now();
            
            tryMergeAndRecur(vals, exprs, (double)target, solutions);
            if (!solutions.empty()) {
                cout << "  Found " << solutions.size() << " solution(s) for one combination of " << left << endl;
